...
class BinaryLogReg:
    ...
    def mu(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        pred_y = X @ self.weight
        return 1 / (1 + np.exp((-y) * (self.bias + pred_y)))
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        J=  np.mean(np.log(1 + np.exp(-y * (self.bias + X @ self.weight)))) + self._lambda * np.linalg.norm(self.weight, 2)**2
        return J
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        # found in Q7a
        mu = self.mu(X,y)
        loss = np.mean((1 - mu) * (-y * X.T), axis=1) + 2 * self._lambda * self.weight
        return loss
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        # if the predicted value is > 0, then the event is true (1)
        # if the predicted value is <= 0, then the event is false (-1)
        return np.where(X @ self.weight > 0, 1, -1)
    ...
    def misclassification_error(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        accuracy_rate = np.sum(pred_y == y) / len(X)
    ...
    def step(self, X: np.ndarray, y: np.ndarray, learning_rate: float = 1e-4):
        ...
        # step size = learning rate * slope (aka. gradient of that parameter)
    ...
    def train(
        ...
        for i in range(epochs):
            for j in range(num_batches):
                # get random samples we want to use for our training
                num_rows = len(X_train)
                curr_batch = RNG.choice(num_rows, batch_size, replace = False)
                X_batch = X_train[curr_batch]
                y_batch = y_train[curr_batch]
                self.step(X_batch, y_batch, learning_rate)
            
            # put loss and error of train and test into the dictionary
        ...
        # epoch - train full dataset once
        # batch_size = subset of data
        # for 7b: do gradient descent on full data set

if __name__ == "__main__":
    model = BinaryLogReg()
    
    (x_train, y_train), (x_test, y_test) = load_2_7_mnist()
    #history = model.train(x_train, y_train, x_test, y_test, 0.4, epochs = 30, batch_size = len(x_train)) # gradient
    #history = model.train(x_train, y_train, x_test, y_test, 0.001, epochs = 30, batch_size = 1) # stochastic
    history = model.train(x_train, y_train, x_test, y_test, 0.01, epochs = 100, batch_size = 100) # mini-batch
