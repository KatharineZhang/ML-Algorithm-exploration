...
class CrossEntropyLossLayer(nn.Module):
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        ...
        """
        # for every row of predictions, get the y'th column and take the -log of it
        row_indices = torch.arange(len(y_pred))
        return -torch.log(y_pred[row_indices, y_true]).mean()
