...
def train(
    ...
    for epoch in range(epochs):
        ...
        accuracy_1 = total_loss / len(train_loader)
        history["train"].append(accuracy_1)

        if (val_loader is not None):
            model.eval()
            val_loss = 0.0
            with torch.no_grad():
                for sample, expected in val_loader:
                    prediction = model(sample)
                    loss = criterion(prediction, expected)
                    val_loss += loss.item()
            accuracy_2 = val_loss / len(val_loader)
            history["val"].append(accuracy_2)
    return history
